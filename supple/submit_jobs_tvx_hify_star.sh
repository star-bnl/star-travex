...
for TVX_DETECTOR in "${TVX_DETECTOR_LAYERS[@]}"
do

   OUT_DIR="${TVX_OUT_PREFIX}/$TVX_DETECTOR"
   mkdir -p $OUT_DIR

   echo
   echo "Submitting job for $TVX_DETECTOR"
   star-submit-template -template $TVX_SOURCE_DIR/supple/job_template_tvx_hify_star.xml \
      -entities JOB_INPUT_FILE=$JOB_INPUT_FILE,JOB_NEVENTS=$JOB_NEVENTS,JOB_BFC_OPTIONS=$JOB_BFC_OPTIONS,OUT_DIR=$OUT_DIR,TVX_DETECTOR=$TVX_DETECTOR,TVX_DEACTIVATE_DETECTOR=$TVX_DEACTIVATE_DETECTOR
   echo
done
