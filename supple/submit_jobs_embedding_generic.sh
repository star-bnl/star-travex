...
# Optional path to directory with *.fzd files. If you choose to use available
# files make sure their names correspond to the file names in $INPUT_FILE_LIST
: ${FZD_DIR:="./"}
...
#JOB_BFC_OPTIONS_3="pp2012b btof fmsDat fgt fgtPoint $JOB_BFC_OPTIONS_VERTEX BEmcChkStat Corr4 OSpaceZ2 OGridLeak3D -evout IdTruth TpxClu -hitfilt Embedding TpcMixer McAna -in NoInput useInTracker btofsim emcSim EEfs"
 JOB_BFC_OPTIONS_3="pp2012b AgML mtdDat btof fmsDat $JOB_BFC_OPTIONS_VERTEX BEmcChkStat Corr4 OSpaceZ2 OGridLeak3D -evout geantout IdTruth TpxClu -hitfilt Embedding TpcMixer McAna -in NoInput useInTracker btofsim emcSim EEfs"
...
rsync -av $BFC_MIXER_MACRO  $STAR_TRAVEX_INSTALL_DIR/EmbeddingReco.C
rsync -av $VERTEX_GEN_MACRO $STAR_TRAVEX_INSTALL_DIR/GenerateVertex.C
rsync -av $STARSIM_KUMAC    $STAR_TRAVEX_INSTALL_DIR/starsim.kumac
...
# In STAR framework LIB is used for optimized libraries, therefore, create an alias
...
COMMAND="star-submit-template -template $SOURCE_DIR/supple/job_template_embedding_generic.xml \
   -entities  OUT_DIR=$OUT_DIR,INPUT_FILE_LIST=$INPUT_FILE_LIST,RANDOM_SEED=$RANDOM_SEED,JOB_BFC_OPTIONS_1=$JOB_BFC_OPTIONS_1,JOB_BFC_OPTIONS_2=$JOB_BFC_OPTIONS_2,JOB_BFC_OPTIONS_3=$JOB_BFC_OPTIONS_3,REQUEST_UID=$REQUEST_UID,STAR_TRAVEX_INSTALL_DIR=$STAR_TRAVEX_INSTALL_DIR,FZD_DIR=$FZD_DIR"
